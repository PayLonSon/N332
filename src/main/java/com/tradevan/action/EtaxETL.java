...
public class EtaxETL extends LogAction {
	...
	public void run() {
		logAction.setLogget(EtaxETL.class);
	...
	private void compareText(Set<List<String>> item) {
		...
		String getItemSQL = String
				.format("SELECT distinct [Row],[DESCRIPTION] FROM [ChemiBigData].[dbo].[廠商關聯與流向_財資proc3]");
		indexs.add(1);
		indexs.add(2);
	...
	private void insertInto(Set<List<String>> compareData) {
		...
		List<String> l = null;
		String insertSql = String.format("INSERT INTO [廠商關聯與流向_化學物質proc1]( [CASNo],[財資_ROW], [關鍵字]) VALUES (?,?,?); ");
